...
document.addEventListener("keydown", (event) => {
    if (event.key === "Tab") {
        eid("download-button").value = 'Download'
        eid("download-button").style.padding = '0 1rem'
    }
})
function button() {
    ...
    regexTest ? changeDownloadButton(1, 'Download') : changeDownloadButton(0, 'Download');
}
function clearInput() {
...
async function loadOnDemand(elementId, blockId) {
    ...
    eid(elementId).innerHTML = `<div class="loader">wait</div>`;
